...
def compute_nmf(data, rank, U_in, V_in, n_iter_max=100, tol=1e-8,
    ...
    for iteration in range(n_iter_max):
        ...
        # Modified: tolerance on error itself rather than differences
        if iteration > 0 and cost_fct_vals[-1] < tol:
...
def one_nmf_step(data, rank, U_in, V_in, norm_data, update_rule, beta,
    ...
    if update_rule == "hals":
        ...
        cost = np.sqrt(np.abs(norm_data**2 - 2*np.sum(UtM*V) + np.sum(UtU*(V@V.T))) )/np.prod(data.shape)
