...
algs = ["MU_KL", "fastMU_KL_trueH_min", "fastMU_KL_trueH", "fastMU_KL_min", "fastMU_KL"]
if len(sys.argv)==1:
    nb_seeds = 0 #no run
else:
    nb_seeds = int(sys.argv[1])  # Change this to >0 to run experiments

name = "KL_sparse_run_01-17-2023"
@run_and_track(algorithm_names=algs, path_store="Results/", name_store=name,
                add_track = {"distribution" : "uniform"},
                nb_seeds=nb_seeds,
                mnr = [[100,50,5]],#[[200,100,5]],#,[1000,400,20]],
                NbIter_inner = [50], # for Lee and Seung also
                SNR = [100],
                delta = 0.1,
                seeded_fun=True,
...
def one_run(mnr=[100,100,5],SNR=50, NbIter=100, tol=0, NbIter_inner=10, verbose=False, show_it=100, delta=0, seed=1, epsilon=1e-8, setup=1):
    ...
    error1, W1, H1, toc1, cnt1 = nmf_kl.Proposed_KL(V, Wini, Hini, NbIter=NbIter, nb_inner=NbIter_inner, tol=tol, verbose=verbose, print_it=show_it, delta=delta, use_LeeS=True, gamma=1, true_hessian=True, epsilon=epsilon)
    error2, W2, H2, toc2, cnt2 = nmf_kl.Proposed_KL(V, Wini, Hini, NbIter=NbIter, nb_inner=NbIter_inner, tol=tol, verbose=verbose, print_it=show_it, delta=delta, use_LeeS=False, gamma=1.9, true_hessian=True, epsilon=epsilon)
    ...
    return {"errors" : [error0, error1, error2, error3, error4], 
            ...
            "cnt" :  [cnt0[::10], cnt1[::10], cnt2[::10], cnt3[::10], cnt4[::10]]
           }
