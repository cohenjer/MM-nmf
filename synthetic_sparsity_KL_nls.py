...
@run_and_track(algorithm_names=algs, path_store="Results/", name_store=name,
                ...
                SNR = [100],
                delta = 0,
                Hsparsity = [0.01, 0.1, 0.3, 0.5, 1],
                seeded_fun=True,
                )
def one_run(mnr=[100,100,5],SNR=50, NbIter=3000, verbose=True, show_it=100, delta=0, seed=1, Hsparsity=1):
    m, n, r = mnr
    # Fixed the signal 
    rng = np.random.RandomState(seed+20)
    Worig = rng.rand(m, r) 
    Horig = rng.rand(r, n)
    Horig = sparsify(Horig,Hsparsity)  
    ...
    error1, H1, toc1 = nls_kl.Proposed_KL(V, Worig, Hini, NbIter=NbIter, verbose=verbose, print_it=show_it, delta=delta, use_LeeS=True)
